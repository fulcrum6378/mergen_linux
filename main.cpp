...
int main() {
    ...
    // 5. Query the buffer to get raw data, e.g. ask for the buffer you requested
...
/***************************** Begin looping here *********************/
    ...
    // Frames get written after de-queuing the buffer

    cout << "Buffer has: " << (double) buffer_info.bytesused / 1024
         << " KBytes of data" << endl;
    ...
    __u32 bufPos = 0, outFileMemBlockSize = 0; // the position in the buffer and the amount to copy from
    // the buffer
    __u32 remainingBufferSize = buffer_info.bytesused; // the remaining buffer size, is decremented by
    // memBlockSize amount on each loop, so we do not overwrite the buffer
    char *outFileMemBlock;  // a pointer to a new memory block
